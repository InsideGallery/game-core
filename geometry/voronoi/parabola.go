...
func GetParabolaABC(focus *Site, yOfDirectrix int) (float64, float64, float64) {
	...
	// And the distance between the focus and the direct is two times this distance,
	// so (y_{f} - y_{d}) = 1/(2a), which simplifies to the formula above.
	a := 1.0 / (2.0 * float64(focus.Y-yOfDirectrix)) //nolint:mnd

	// b = -2ax_{f}
	// Calculation of b is based on the vertex form of the parabola equation: x_{0} = -b/(2a)
	b := -2.0 * a * float64(focus.X) //nolint:mnd
	...
	c := a*math.Pow(float64(focus.X), 2) + float64(focus.Y) - 1/(4*a) //nolint:mnd
...
func GetXOfIntersection(left *Node, right *Node, directrix int) (int, error) {
	...
	if leftFocus.Y == rightFocus.Y {
		return (leftFocus.X + rightFocus.X) / 2, nil // nolint:mnd
	}
	...
	discriminant := b*b - 4*a*c                       // nolint:mnd
	root1 := (-b + math.Sqrt(discriminant)) / (2 * a) // nolint:mnd
	root2 := (-b - math.Sqrt(discriminant)) / (2 * a) // nolint:mnd
