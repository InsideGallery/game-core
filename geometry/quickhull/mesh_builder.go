...
type meshBuilder struct {
	...
	// Insted, they are marked as disabled which means that the indices can
	// be reused when we need to add new Faces and half edges to the mesh.
...
func newMeshBuilder(a, b, c, d int) meshBuilder {
	...
	acd := meshBuilderFace{
		halfEdgeIndex: 3, //nolint:mnd
	}
	mb.faces = append(mb.faces, acd)

	bad := meshBuilderFace{
		halfEdgeIndex: 6, //nolint:mnd
	}
	mb.faces = append(mb.faces, bad)

	cbd := meshBuilderFace{
		halfEdgeIndex: 9, //nolint:mnd
	}
...
type meshBuilderFace struct {
	...
	horizonEdgesOnCurrentIteration byte
