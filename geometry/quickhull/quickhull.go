...
type QuickHull struct {
	...
	planar               bool
	planarPointCloudTemp []shapes.Point
	vertexData           []shapes.Point
	mesh                 meshBuilder
...
type diagnostics struct {
	// How many times QuickHull failed to solve the horizon edge.
	//	// Failures lead to degenerated convex hulls.
...
// ConvexHullAsMesh calculates the convex hull of the given point
// cloud using the Quickhull algorithm and returns it as a HalfEdgeMesh.
// If epsilon is <= 0 a default value will be used.
...
func (qh *QuickHull) buildMesh(pointCloud []shapes.Point, epsilon float64) {
	if len(pointCloud) == 0 {
		return
	}
	...
	scale := scale(qh.vertexData, qh.extremeValueIndices)
...
// createConvexHalfEdgeMesh this will update m_mesh from which
// we create the ConvexHull object that getConvexHull function returns
// should we add cleanup at the end (m_indexVectorPool.clear();)?
func (qh *QuickHull) createConvexHalfEdgeMesh() { //nolint:gocyclo
	...
	assertTrue(len(qh.mesh.faces) == 4) //nolint:mnd
	...
	for len(faceList) > 0 {
		...
		if iter == maxInt {
			...
			// and the max value represents unvisited Faces. At this point
			// we have to reset iteration counter. This shouldn't be an
				...
				if d > 0 {
					...
					for _, heIndex := range qh.mesh.halfEdgeIndicesOfFace(*pvf) {
						...
						if opp != fd.enteredFromHalfEdge {
							possiblyVisibleFaces = append(possiblyVisibleFaces,
								faceData{
									faceIndex:           qh.mesh.halfEdges[opp].Face,
		...
		// This may fail due to numerical instability in
		// which case we give up trying to solve horizon edge
		// for this point and accept a minor degeneration in the convex hull.
		...
		for _, faceIdx := range visibleFaces {
			...
			// on the positive side of it. We need to assign those points
		...
		if nDisabled < nHorizontalEdges*2 { //nolint:mnd
			nNewHalfEdgesNeeded := nHorizontalEdges*2 - nDisabled //nolint:mnd
		...
		for i := 0; i < nHorizontalEdges; i++ {
			...
			var idx int
			if i > 0 {
				idx = i*2 - 1 //nolint:mnd
			} else {
				idx = 2*nHorizontalEdges - 1 //nolint:mnd
			}

			qh.mesh.halfEdges[ca].Opp = qh.newHalfEdgeIndices[idx]
			qh.mesh.halfEdges[bc].Opp = qh.newHalfEdgeIndices[((i+1)*2)%(nHorizontalEdges*2)] //nolint:mnd
		}
...
// initialTetrahedron create a half edge mesh representing
// the base tetrahedron from which the QuickHull iteration proceeds.
// m_extremeValues must be properly set up when this is called.
func (qh *QuickHull) initialTetrahedron() meshBuilder {
	...
	if nVertices <= 3 { //nolint:mnd
		v := [4]int{ //nolint:mnd
			0,
			int(math.Min(1, float64(nVertices-1))),
			int(math.Min(2, float64(nVertices-1))), //nolint:mnd
			nVertices - 1,
		}

		n := triangleNormal(qh.vertexData[v[0]], qh.vertexData[v[1]], qh.vertexData[v[2]]) //nolint:mnd
	...
	for i := 0; i < 6; i++ {
		for j := i + 1; j < 6; j++ {
			dv := qh.vertexData[qh.extremeValueIndices[i]].Subtract(qh.vertexData[qh.extremeValueIndices[j]])
			dSq := dv.NormalSquare()
	...
	if maxD == qh.epsilonSquared {
		...
		return newMeshBuilder(0,
			int(math.Min(1, float64(nVertices-1))), //nolint:mnd
			int(math.Min(2, float64(nVertices-1))), //nolint:mnd
			int(math.Min(3, float64(nVertices-1))), //nolint:mnd
		)
	}
	...
	r := newRay(qh.vertexData[p1], qh.vertexData[p2].Subtract(qh.vertexData[p1]))
	maxD = qh.epsilonSquared
	maxI := maxInt
	...
	if maxD == qh.epsilonSquared {
		...
		// subspace of R^3: convex hull has no volume => return a thin triangle
	...
	baseTriangleVertices := [3]shapes.Point{
		qh.vertexData[baseTriangle[0]],
		qh.vertexData[baseTriangle[1]],
		qh.vertexData[baseTriangle[2]],
	}
	...
	maxI = 0
	{
		...
		// Enforce CCW orientation (if soul prefers clockwise orientation,
...
// addPointToFace associates a point with a Face if the point resides
// on the positive side of the plane. Returns true if the points was on the positive side.
...
// reorderHorizontalEdges given a list of half edges,
// try to rearrange them so that they form a loop. Return true on success.
